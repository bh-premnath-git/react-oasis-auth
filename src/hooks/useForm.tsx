...
export const useForm = () => {
    const { setIsFormOpen, setSelectedSchema } = usePipelineContext();
    const [formData, setFormData] = useState<any>(null);

    const openForm = useCallback((schema: any, initialValues?: any) => {
        setSelectedSchema({...schema, initialValues });
        setIsFormOpen(true);
    }, [setSelectedSchema, setIsFormOpen]);

    const closeForm = useCallback(() => {
        setIsFormOpen(false);
    }, [setIsFormOpen]);

    return { formData, setFormData, openForm, closeForm };
};